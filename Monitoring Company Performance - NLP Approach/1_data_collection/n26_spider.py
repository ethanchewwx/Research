...
class DigitalBankSpiderXPath(scrapy.Spider):
    ...
    # start_urls = ['https://uk.trustpilot.com/review/n26.com']
    start_urls = ['https://uk.trustpilot.com/review/n26.com?languages=all']
    ...
    def parse(self, response):
        ...
        for review in response.xpath('//div[@class="paper_paper__1PY90 paper_square__lJX8a card_card__lQWDv '
            ...
            if date is None:
                date = rating_date.xpath('.//div[@class="typography_typography__QgicV typography_bodysmall__irytL '
                                         'typography_color-gray-6__TogX2 typography_weight-regular__TWEnf '
                                         'typography_fontstyle-normal__kHyN3 styles_datesWrapper__RCEKH"]/span/time/'
                                         '@datetime').get()
            ...
            yield {
                ...
                'rating': rating_date.xpath('.//div[@class="star-rating_starRating__4rrcf star-rating_medium__iN6Ty"]/'
                                            'img/@alt').extract(),
                'date': date,
                'title': str(title_text.xpath('./h2/a/text()').extract()),
                'text': str(title_text.xpath('.//p/text()').extract()),
                'company_reply': str(review.xpath('.//div[@class="styles_wrapper__ib2L5"]/'
